...
begin
   ...
   k:=0;
   for i:=0 to 96 do k:=k+abs(k1[i]-k2[i]);
   for i:=123 to 1001 do k:=k+abs(k1[i]-k2[i]);
   if (k=0) and (s1<>'') and (s2<>'') then srv:=true
   else srv:=false;
end;
begin
   ...
   if (false) and (n=6) and (a[1] = '10713587.cs') and (a[2] = '49_38a.cpp')
            ...
            begin
               ...
               while not eof(f) do
                  begin
                     ...
                     and} (pos('::',s1)=0) and (pos('printf',s1)=0)
                     and (pos('scanf',s1)=0)//and (pos('();',s1)=0)
                     then s:=s+' '+s1;
                  end;
               close(f);
               //s:=lowercase(s);
               ...
               for j:=1 to length(s) do
                  ...
                  and (s[j]<>',') and (s[j]<>'_') and (not (s[j] in ['0'..'9']))
                  and (s[j]<>'.') and (s[j]<>'#')
                  and (s[j]<>'$') and (s[j]<>'`') and (s[j]<>'+') and (s[j]<>'=')
                  and (s[j]<>'-') and (s[j]<>'/') and (s[j]<>'*') and (s[j]<>'>')
                  and (s[j]<>'<') and (s[j]<>':') and (s[j]<>'\') and (s[j]<>'%')
                  then vs[i]:=vs[i]+s[j];
               for j:=1 to 200 do

...
struct interval
{
    ...
    void get(ll v, ll l, ll r, ll tl, ll tr, ll &sum, ll &ans, bool &bit)
    {
        if (tl > tr)
        {
            sum = 0;
        ...
        else
        {
            ...
            bool bit1, bit2, bit;
            m = (l + r) / 2;
            get(v + v, l, m, tl, min(tr, m), sum1, ans1, bit1);
            get(v + v + 1, m + 1, r, max(tl, m + 1), tr, sum2, ans2, bit2);
            sum = sum1 + sum2;
            bit = bit1 & bit2;
            ans = ans2 + sum1 * (r - m - 1);
            if (bit2)
        ...
        for (j = 0; j < DL2; j++)
            {
                ...
                a[j].get(1, 1, n, 1, n, ans1, x, y);
                ans += x * st;
                cout << a[j].tsum[1] << endl;
                val /= 2;
                st *= 2;
            }
        printf("%I64d\n", ans);
